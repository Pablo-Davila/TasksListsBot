...
async def showList(cid, listName):
	...
	if(listName in dic.keys()):
		...
		bot.send_async_message(
			cid, f"La lista {listName} no existe."
        )
...
async def deleteTask(cid, listName, taskNumber):
	...
	try:
		...
		bot.send_async_message(
			cid, "No ha indicado un número de tarea válido.")
		return
	...
	if listName in dic.keys():
		...
		try:
			...
			writeLists(cid, dic)
			bot.send_async_message(
				cid, f"La tarea \"{taskName}\" ha sido eliminada."
            )
		except:
			bot.send_async_message(
				cid, f"Índice fuera de rango: {num}."
            )
	else:
		bot.send_async_message(
			cid, f"La lista {listName} no existe.")
	...
	if listName in dic.keys():
		try:
			...
			try:
				...
				writeLists(cid, dic)
				bot.send_async_message(
					cid, f"Tarea \"{taskName}\" marcada como hecha."
                )
			except IndexError:
				bot.send_async_message(
					cid, "Índice fuera de rango."
                )
			except Exception as e:
				bot.send_async_message(cid, "Internal error, try again")
				print(e)
		except:
			bot.send_async_message(
				cid,
...
async def addAll(cid, listName, tasks):
	...
	if listName in dic.keys():
		...
		writeLists(cid, dic)
		bot.send_async_message(
			cid, f"Se han añadido {c} tareas a la lista \"{listName}\"."
        )
	else:
		bot.send_async_message(
			cid, f"La lista {listName} no existe."
...
async def command_delList(message):
	...
	if listName in dic.keys():
		dic.pop(listName)
		writeLists(cid, dic)
		bot.send_async_message(cid, f"La lista {listName} ha sido eliminada.")
	elif listName == "":
		bot.send_async_message(cid, "Debe indicar la lista que eliminar.")
	else:
		bot.send_async_message(cid, f"La lista {listName} no existe.")
...
async def command_empty(message):
	...
	if listName in dic.keys():
		...
		bot.send_async_message(cid, "Debe indicar la lista que vaciar.")
	else:
		bot.send_async_message(cid, f"La lista {listName} no existe.")
...
async def command_done(message):
	...
	if(len(partes) < 2):
		...
		try:
			...
			bot.send_async_message(cid, "No ha indicado un número de tarea válido.")
			return
